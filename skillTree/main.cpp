...
//driver to test general tree. 
void testGenTree() {
	bool addSuccess = false;
	SkillTree newTree("ButtTree");

	newTree.AddSkill("buttSkillA", "A butt Skill", 0);
	newTree.AddSkill("buttSkillB", "B butt Skill", 0);
	newTree.AddSkill("buttSkillC", "C butt Skill", 0); 
	...
	testFindC = newTree.FindSkill("buttSkillC");
	testFindC->setOstream(&cout);
	testFindC->Display();

	//this should be the bottom of the tree. 
	testFindA = newTree.FindSkill("buttSkillA");
	testFindA->setOstream(&cout);
	testFindA->Display();

	//Should be Bad...
	testFindX = newTree.FindSkill("crappy");
	
	//addSuccess = testTree.addChild(testSkill);
	cout << addSuccess;

	//addSuccess = testTree.addChild(testSkill2);
	cout << addSuccess;
